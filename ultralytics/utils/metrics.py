...
class SegPoseMetrics(SimpleClass):
    """
    Calculates and aggregates detection, segmentation, and pose metrics over a given set of classes.
    ...
    def process(self, tp, tp_m, tp_p, target_cls_p, conf, pred_cls, target_cls):
        """
        ...
        Args:
            ...
            pred_cls (list): List of predicted classes.
            target_cls (list): List of target classes.
            target_cls_p (list): List of target classes for Pose.
        """
        ...
        self.seg.update(results_mask)
        # breakpoint()
        # Keep only person detections, because that's what Pose works on.
        pose_detections = pred_cls == 0
        pred_cls_p = pred_cls[pose_detections]
        conf_p = conf[pose_detections]
        results_pose = ap_per_class(
            tp_p,
            conf_p,
            pred_cls_p,
        ...
        self.pose.nc = target_cls_p.max() + 1
        self.pose.update(results_pose)
        results_box = ap_per_class(
            tp,
            conf,
    ...
    def class_result(self, i):
        """Returns classification results for a specified class index."""
        return (self.box.class_result(i) + self.seg.class_result(i)
                + (self.pose.class_result(i) if i < self.pose.nc else (0.0, 0.0, 0.0, 0.0)))
    ...
    @property
    def fitness(self):
        """Get the fitness score for segmentation, pose, and bounding box models."""
        return self.seg.fitness() + self.box.fitness() + self.pose.fitness()
